...
class MemoryReportNode(Node):
    ...
    def get_size(self, node):
        ...
        size = round((float(node.size) / float(1024)) / 1024, 2)
        return size
...
def current_size_percentage(current_size, root_size):
    """Calculates the current object size percentage compared to the total memory."""
    # Convert to MB.
    root_size = float(root_size) / 1024 / 1024
    return round(float(current_size) / root_size * 100, 2)
...
if __name__ == '__main__':
    ...
    if args['name']:
        filename = '/data/memory_' + args['name'] + '.pkl'
    ...
    for i, line in report_sorted:
        ...
        print "{:<15}".format(current_size_percentage(line['size'], root.size))
